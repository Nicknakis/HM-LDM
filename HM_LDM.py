...
class HM_LDM(nn.Module):
    def __init__(self,sparse_i,sparse_j, input_size,latent_dim,sample_size,non_sparse_i=None,non_sparse_j=None,sparse_i_rem=None,sparse_j_rem=None,scaling=None,norm_values=False):
        ...
        # PARAMETERS
        self.latent_z1=nn.Parameter(torch.randn(self.input_size,latent_dim,device=device))
    ...
    def LSM_likelihood_bias(self,epoch):
        ...
        if self.norm_values:
            ...
            if self.scaling:
                ...
                mat=torch.exp(-self.reg_l*((torch.cdist(self.latent_z+1e-06,self.latent_z,p=2)+1e-06)**2))
                # Non-link N^2 likelihood term
                z_pdist1=0.5*torch.mm(torch.exp(self.gamma.unsqueeze(0)),(torch.mm((mat-torch.diag(torch.diagonal(mat))),torch.exp(self.gamma).unsqueeze(-1))))
                # Likehood link term
                z_pdist2=(-self.reg_l*((((self.latent_z[self.sparse_i_idx]-self.latent_z[self.sparse_j_idx]+1e-06)**2).sum(-1)))+self.gamma[self.sparse_i_idx]+self.gamma[self.sparse_j_idx]).sum()
...
total_runs=5
# Simplex sizes
deltas=[0.01,0.1,1,5,10,15,20,25,50,200]
...
for run in range(1,total_runs+1):
    for latent_dim in latent_dims:
        ...
        for delta in deltas:
            ...
            # idex =np.loadtxt("C:/nnak/Social Network Analysis/facebook_combined.txt")
            ...
            model = HM_LDM(sparse_i=sparse_i,sparse_j=sparse_j,input_size=N,latent_dim=latent_dim,sample_size=sample_size,non_sparse_i=non_sparse_i,non_sparse_j=non_sparse_j,sparse_i_rem=sparse_i_rem,sparse_j_rem=sparse_j_rem,norm_values=False).to(device)         
            ...
            torch.save(model.gamma.detach(),dataset+f'/{dataset}_K_{latent_dim}_delta_{delta}_gamma_{run}')
            torch.save(model.latent_z.detach(),dataset+f'/{dataset}_K_{latent_dim}_delta_{delta}_zeta_{run}')
            roc,pr=model.link_prediction() #perfom link prediction and return auc-roc, auc-pr
            ...
            filename_roc=dataset+f"/ROC_K_{latent_dim}_delta_{delta}_{run}"+".txt"
            filename_pr=dataset+f"/PR_K_{latent_dim}_delta_{delta}_{run}"+".txt"
